...
- memories::Union{Array{Int32,2},Nothing} : optional stored patterns (N×K) as ±1
"""
...
# Convenience constructor accepting AbstractArrays and optional memories keyword
function MyClassicalHopfieldNetworkModel(W::AbstractMatrix, b::AbstractVector, energy::Dict{Int,Float32}; memories=nothing)
    Wf = Float32.(Array(W))
    bf = Float32.(Array(b))
    mem = memories === nothing ? nothing : Array{Int32,2}(memories)
    # enforce zero diagonal (safety)
...
# Lightweight pretty-print
Base.show(io::IO, m::MyClassicalHopfieldNetworkModel) = print(io,
    "MyClassicalHopfieldNetworkModel(N=$(size(m.W,1)), K=$(m.memories === nothing ? length(m.energy) : size(m.memories,2)))"
)
